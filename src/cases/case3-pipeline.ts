...
export class Case3Pipeline {
  ...
  constructor() {
    ...
    this.transformQueue = new PQueue({ concurrency: this.TRANSFORM_CONCURRENCY });
  ...
  public async execute(limit?: number): Promise<PerformanceMetrics> {
    ...
    try {
      ...
      await targetPool.query('DELETE FROM transformed_users WHERE etl_case_number = 3');
    ...
    metrics.thread_count = this.EXTRACT_CONCURRENCY + this.TRANSFORM_CONCURRENCY + this.LOAD_CONCURRENCY;
    metrics.batch_count = Math.ceil(processedCount / this.BATCH_SIZE);
    
    return metrics;
  }

  private async processBatch(offset: number, limit: number): Promise<{ processed: number; errors: number }> {
    let processed = 0;
    let errors = 0;
          ...
          this.transformQueue.add(async () => {
            try {
              ...
              await this.loadQueue.add(async () => {
                ...
                if (processed % 100 === 0) {
                  this.logger.logProgress(processed, limit, `Batch ${offset / this.BATCH_SIZE + 1}`);
                }
              });
